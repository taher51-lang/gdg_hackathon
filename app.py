...
def dispatch_best_hospital(user_lat, user_lon, required_resources: List[str]) -> dict:
    ...
    # 4. AGENTIC MATCHMAKING: Ask the LLM to figure out the best fit
    decision = matchmaker_chain.invoke({
        "resources": ", ".join(required_resources),
        "units_list": hospitals_text
    })
...
# Global store for incidents to allow the dashboard to monitor live data
incident_history = []
...
async def triage_and_dispatch(request: EmergencyRequest):
    try:
        ...
        # Save to history for the dashboard to monitor
...
def get_incidents():
    """Returns the list of all processed incidents for the monitoring dashboard."""
